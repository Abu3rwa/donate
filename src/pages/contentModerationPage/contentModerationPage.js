  ...
  const handleAction = async (actionType, id, comment) => {
    try {
      ...
      if (actionType === "approve") {
        await moderationService.approveItem("expense", id, comment, "مشرف");
        successMessage = "تمت الموافقة على المصروف بنجاح.";
      } else if (actionType === "reject") {
        await moderationService.rejectItem("expense", id, comment, "مشرف");
      ...
      >
        <header className="mb-6">
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white">
            مراجعة المحتوى (Content Moderation)
          </h1>
        ...
        <div className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm mb-6 flex flex-col md:flex-row gap-3">
          ...
          <div className="grid grid-cols-2 md:flex gap-3">
            <select
              className="w-full text-center px-7 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500"
