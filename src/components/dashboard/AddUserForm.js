...
import { createUserDocument, createUserByAdmin } from "../../services/userService";
...
export default function AddUserForm({ onCancel, onUserAdded, initialData }) {
  ...
  const [permissions, setPermissions] = useState(() => {
    ...
    if (initialData && initialData.adminType) return ADMIN_PERMISSIONS[initialData.adminType]?.permissions || [];
    return [];
  });
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const userProfile = {
        ...
        source: "website",
        status: "Active",
        role: data.role === "مسؤول" ? ADMIN_PERMISSIONS[data.adminType]?.name || "مدير" : data.role,
  ...
  return (
    <form className="space-y-6 p-4 py-8" noValidate onSubmit={handleSubmit(onSubmit)}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <input id="firstName" type="text" {...register("firstName")} className="input-field" />
        ...
        <div>
          ...
          <input id="lastName" type="text" {...register("lastName")} className="input-field" />
      ...
      <div>
        ...
        <input id="phone" type="tel" {...register("phone")} className="input-field" pattern="[0-9]{8,15}" inputMode="tel" />
      ...
      <div>
        ...
        <input id="photoURL" type="url" {...register("photoURL")} className="input-field" />
      ...
      <div>
        ...
        <div className="flex gap-2">
          <input id="password" type="text" {...register("password")} className="input-field flex-1" autoComplete="new-password" />
          <button type="button" className="rounded bg-indigo-500 text-white px-3 py-1 text-xs font-semibold hover:bg-indigo-700 transition-colors" onClick={() => setValue("password", generateEasyPassword(6))} tabIndex={-1} >
            توليد كلمة مرور
          </button>
        </div>
      ...
      <div>
        ...
        <select id="role" {...register("role")} className="input-field text-center">
      ...
      {role === "مسؤول" && (
        <>
          <div>
            <label htmlFor="adminType" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1" >
              نوع المسؤول
            </label>
            <select id="adminType" {...register("adminType")} className="input-field text-center" >
            ...
            {errors.adminType && ( <p className="error-message">{errors.adminType.message}</p> )}
          </div>
          <div className="mt-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {allPermissions.map((perm) => (
                <label key={perm} className="flex items-center gap-2 text-sm">
                  <input type="checkbox" checked={permissions.includes(perm)} onChange={e => {
                      if (e.target.checked) {
      ...
      <div className="flex flex-col-reverse sm:flex-row sm:justify-end sm:gap-4">
        <button type="button" onClick={onCancel} className="mt-2 sm:mt-0 w-full sm:w-auto justify-center rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-4 py-2 text-base font-medium text-gray-700 dark:text-gray-200 shadow-sm hover:bg-gray-50 dark:hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-400 dark:focus:ring-offset-gray-800 transition-colors" >
          إلغاء
        </button>
        <button type="submit" disabled={isLoading} className="w-full sm:w-auto justify-center rounded-md border border-transparent bg-indigo-600 px-4 py-2 text-base font-medium text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 dark:focus:ring-offset-gray-800 transition-colors disabled:bg-indigo-400 disabled:cursor-not-allowed" >
