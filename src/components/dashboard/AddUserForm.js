  ...
  const onSubmit = async (data) => {
    ...
    if (initialData && (initialData.uid || initialData.id)) {
      ...
      const updatedUser = {
        ...
        adminType: isAdmin ? data.adminType : undefined,
        adminLevel: isAdmin ? adminPerm?.level : undefined,
        permissions: isAdmin ? adminPerm?.permissions : [],
      };
    ...
    try {
      ...
      try {
        ...
        toast.error(
          "فشل إنشاء المستخدم في المصادقة: " + (err.message || err.code)
        );
        setIsLoading(false);
        // Do not reset or close the form, allow user to retry
        return;
      }
      ...
      try {
        await createUserDocument(uid, userProfile);
        ...
        toast.error("فشل حفظ بيانات المستخدم: " + (err.message || err.code));
        setIsLoading(false);
        // Do not reset or close the form, allow user to retry
        return;
      }
        ...
        <div className="mx-auto flex items-center justify-center h-12 w-12 rounded-full bg-indigo-100 dark:bg-indigo-900/50">
          ...
          >
            <path
              ...
              strokeWidth={2}
              d="M16 14v6M8 14v6M12 14v6M12 10a4 4 0 100-8 4 4 0 000 8z"
            />
          </svg>
        </div>
