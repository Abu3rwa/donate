...
const QuickActions = ({
  ...
  const getActionHandler = (action) => {
    ...
    if (action.onClickKey === "onShowFinancialReports") return onShowFinancialReports;
    return null;
  };
  ...
  const hasPermissionForAction = (action) => {
    ...
    return user.permissions.includes(action.permission) || user.permissions.includes("all");
  };
    ...
    <div>
      ...
      <div className="flex flex-row flex-wrap gap-3 sm:gap-4 justify-start items-stretch">
        {actions.map((action) => {
          ...
          const colorClass = permitted ? getColorClasses(action.color) : getDisabledColorClasses();
          if (action.link) {
            return (
              <Link
                key={action.name}
                to={permitted ? action.link : "#"}
                className={`flex flex-col items-center justify-center min-w-[100px] max-w-[160px] sm:min-w-[140px] sm:max-w-xs px-4 py-4 sm:px-6 sm:py-5 rounded-xl sm:rounded-2xl shadow transition-colors duration-200 text-sm sm:text-base ${colorClass} ${!permitted ? "opacity-70 pointer-events-none" : ""}`}
              ...
              >
                ...
                <span className="font-semibold  text-center leading-tight">
                  {action.name}
                  {!permitted && (
                    <span className="block text-xs text-red-800 mt-1 font-bold">ليس لديك صلاحية الوصول</span>
                  )}
                </span>
              </Link>
            );
            ...
            return (
              <button
                ...
                className={`flex flex-col items-center justify-center min-w-[100px] max-w-[160px] sm:min-w-[140px] sm:max-w-xs px-4 py-4 sm:px-6 sm:py-5 rounded-xl sm:rounded-2xl shadow transition-colors duration-200 text-sm sm:text-base ${colorClass}`}
                style={{ flex: "1 1 100px", minHeight: "110px" }}
                disabled={!permitted}
              >
                <div className="mb-1 sm:mb-2">{React.cloneElement(action.icon, { className: 'w-12 h-12' })}</div>
                <span className="font-semibold text-center leading-tight">
                  {action.name}
                  {!permitted && (
                    <span className="block text-xs text-gray-800 mt-1 font-bold">ليس لديك صلاحية الوصول</span>
                  )}
                </span>
              </button>
            );
